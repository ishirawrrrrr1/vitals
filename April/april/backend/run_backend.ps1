...
$retries = 0
$serverReady = $false
while ($retries -lt 20 -and !$serverReady) {
    try {
        $check = Invoke-WebRequest -Uri "http://localhost:3000" -Method Head -UseBasicParsing -TimeoutSec 2 -ErrorAction SilentlyContinue
